...
//            trainobj_w.close();
            pass_file_w = new FileOutputStream("passobj.txt");
            passobj_w = new ObjectOutputStream(pass_file_w);
            passobj_w.writeObject(passangers);
            passobj_w.close();

            //train c_train = new train();
            //c_train.add_train("Питер","Москва",4,6,20,1200);
            train_file_w = new FileOutputStream("trainobj.txt");
            trainobj_w = new ObjectOutputStream(train_file_w);
            trainobj_w.writeObject(trains);
            trainobj_w.close();
        }
